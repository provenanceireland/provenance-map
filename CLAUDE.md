...
- **Verified Producers (tier: "verified"):** Green `#4A8A55`, 9px dot, thin white border (`1px solid rgba(248,248,243,0.6)`), static green glow (`box-shadow: 0 0 11px 4.5px rgba(74,138,85,0.61)`), expanding green pulse ring (`pulse-verified` — 4s ease-out, scale 1→5.3, same style as gold seal pulse). Card pill "Provenance Verified" in matching `#4A8A55`. Set `"tier": "verified"` in producers.json.
- **Provenance Founder:** Shown as an additional pill (`data-badge2="Provenance Founder"`) on the card. No special pin treatment — pin appearance is controlled by tier. The Founder pill is suppressed when it would sit beside a tier pill (e.g. Verified). Currently: Saltrock Dairy Farm (badge2 set, but suppressed on card as it sits beside the Verified pill). Newbard Organic Farm Ltd and Staffords Butchers were previously Founders but their badge has been removed and they now render as standard dark-green Discovered pins.
